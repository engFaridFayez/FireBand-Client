...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold sm:text-2xl">Gallery</h1>
        <p class="text-sm text-gray-500 sm:text-base">Manage gallery images.</p>
      </div>

      <button
        class="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 sm:text-base"
        @click="router.push({ name: 'gallery-create' })"
      >
        Add Image
      </button>
    </div>
    ...
    <div
      v-if="!galleryStore.galleryImages.length"
          ...
          <div class="flex gap-4">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-2">
                <p class="truncate font-semibold">{{ image.title || "-" }}</p>
              ...
              <p class="mt-1 text-sm text-zinc-300">
                {{ image.sub_category.name }}
              </p>
              <p class="mt-1 text-xs text-zinc-400">Order: {{ image.order }}</p>
            </div>
          </div>
