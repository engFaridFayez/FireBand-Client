...
const { selectedRule } = storeToRefs(ruleStore);
...
async function loadRule() {
  ...
  form.text = selectedRule.value.text;
  form.order = selectedRule.value.order;
  form.sub_category = String(selectedRule.value.sub_category);
}
...
async function submit() {
  try {
    ...
    await ruleStore.updateRule(Number(route.params.id), data);

    await showPopup("success", "Success", "Rule updated successfully.");
    ...
    await showPopup("error", "Error", "Failed to update Rule.");
  }
}
...
<template>
  <div class="max-w-3xl mx-auto p-6">
    <h1 class="text-2xl font-bold text-white mb-6">Edit Rule</h1>
