...
async function submit() {
  ...
  try {
    await reelStore.createReel(data);
...
<template>
  <div class="mx-auto max-w-3xl rounded-xl bg-amber-950 p-6 shadow">
    <h1 class="mb-6 text-2xl font-bold">Create Reel</h1>
    ...
    <form class="space-y-5" @submit.prevent="submit">
      <div>
        ...
        <select
          ...
          class="w-full rounded-lg border p-3 bg-amber-900"
          required
        >
      ...
      <div>
        ...
        <select v-model="form.platform" class="w-full rounded-lg border p-3 bg-amber-900" >
      ...
      <div>
        ...
        <p class="mt-2 text-sm text-gray-500">
          <template v-if="form.platform === 'youtube'">
            Paste any YouTube link (Watch, Shorts, or youtu.be). It will be
      ...
      <div>
        <label class="mb-2 block font-medium"> Thumbnail (Optional) </label>
